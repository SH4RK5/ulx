...
function xgui.init()
			...
			for _, dtype in ipairs(datatypes) do
				if xgui.dataTypes[dtype] then
					...
					if ULib.ucl.query(ply, data.access) then
						...
						local size = data.maxchunk or
						0                   --Split the table into "chunks" of per-datatype specified size to even out data flow. 0 to disable
	...
	ulx.addToHelpManually("Menus", "xgui",
		"<show, hide, toggle> - Opens and/or closes XGUI. (say: !xgui, !menu) (alias: ulx menu)")
end
