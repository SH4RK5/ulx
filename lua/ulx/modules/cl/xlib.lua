	...
	if t.centericon then --Place the image in the cetner of the button instead of the default layout.
		function pnl:PerformLayout()
			if (IsValid(self.m_Image)) then
				self.m_Image:SetPos((self:GetWide() - self.m_Image:GetWide()) * 0.5,
...
function xlib.makeslider(t)
	...
	function pnl.TextArea:OnEnter()
		pnl.TextArea:SetText(string.format("%." .. (pnl.Scratch:GetDecimals()) .. "f",
			tonumber(pnl.TextArea:GetText()) or 0))
...
function PANEL:UpdateColorText()
	...
	self.ColorCube:SetColor(Color(self.txtR:GetValue(), self.txtG:GetValue(), self.txtB:GetValue(),
		self.showAlpha and self.txtA:GetValue()))
	if (self.showAlpha) then self.AlphaBar:SetBarColor(Color(self.txtR:GetValue(), self.txtG:GetValue(),
			self.txtB:GetValue(), 255)) end
...
function PANEL:PerformLayout()
	self:SetColor(Color(self.txtR:GetValue(), self.txtG:GetValue(), self.txtB:GetValue(),
		self.showAlpha and self.txtA:GetValue()))
end
...
function xlibAnimation:Stop()
	...
	for i, v in ipairs(xlib.activeAnims) do
		if v == self then
			table.remove(xlib.activeAnims, i)
			break
		end
	end
	xlib.animQueue_call()
...
xlib.addToAnimQueue = function(obj, ...)
	...
	if type(obj) == "function" then
		table.insert(outTable, function()
			xlib.animRunning = true
			obj(unpack(arg))
			xlib.animQueue_call()
		end)
	elseif type(obj) == "string" and xlib.animTypes[obj] then
		...
		table.insert(outTable,
			function()
				xlib.animRunning = xlib.animTypes[obj]
...
local function fadeAnim_run(delta, data)
	if data.panelOut then
		data.panelOut:SetAlpha(255 - (delta * 255))
		data.panelOut:SetVisible(true)
	end
	if data.panelIn then
		data.panelIn:SetAlpha(255 * delta)
		data.panelIn:SetVisible(true)
	end
end

local function fadeAnim_start(data)
	if data.panelOut then
		data.panelOut:SetAlpha(255)
		data.panelOut:SetVisible(true)
	end
	if data.panelIn then
		data.panelIn:SetAlpha(0)
		data.panelIn:SetVisible(true)
	end
end
