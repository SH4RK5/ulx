...
function ulx.userallow(calling_ply, target_ply, access_string, access_tag)
	...
	if not success then
		ULib.tsayError(calling_ply,
			string.format("User \"%s\" already has access to \"%s\"", target_ply:Nick(), access_string), true)
	else
		if not access_tag or access_tag == "" then
			ulx.fancyLogAdmin(calling_ply, "#A granted access #q to #T", access_string, target_ply)
		else
			ulx.fancyLogAdmin(calling_ply, "#A granted access #q with tag #q to #T", access_string, access_tag,
				target_ply)
		end
	end
end
...
function ulx.userdenyid(calling_ply, id, access_string, should_use_neutral)
	...
	if should_use_neutral then
		if success then
			...
			ULib.tsayError(calling_ply,
				string.format("User \"%s\" isn't denied or allowed access to \"%s\"", name, access_string), true)
		end
	else
		if not success then
			ULib.tsayError(calling_ply,
				string.format("User \"%s\" is already denied access to \"%s\"", name, access_string), true)
		else
...
function ulx.groupallow(calling_ply, group_name, access_string, access_tag)
	...
	if not success then
		ULib.tsayError(calling_ply,
			string.format("Group \"%s\" already has access to \"%s\"", group_name, access_string), true)
	else
		if not access_tag or access_tag == "" then
			ulx.fancyLogAdmin(calling_ply, "#A granted access #q to group #s", access_string, group_name)
		else
			ulx.fancyLogAdmin(calling_ply, "#A granted access #q with tag #q to group #s", access_string, access_tag,
				group_name)
		end
	end
end
...
function ulx.groupdeny(calling_ply, group_name, access_string)
	...
	if success then
		...
		ULib.tsayError(calling_ply,
			string.format("Group \"%s\" doesn't have access to \"%s\"", group_name, access_string), true)
	end
end
