...
if ULib.fileExists("lua/ulx/modules/cl/motdmenu.lua") or ulx.motdmenu_exists then
	...
	if SERVER then
		...
		ulx.convar("motdfile", "ulx_motd.txt", "MOTD filepath from gmod root to use if ulx showMotd is 1.",
			ULib.ACCESS_ADMIN)
		ulx.convar("motdurl", "ulyssesmod.net", "MOTD URL to use if ulx showMotd is 3.", ULib.ACCESS_ADMIN)
		ulx.convar("motdDisabledMessage", "1",
			"<0/1> - Show disabled message when MOTD command is run if MOTD is disabled by the server. 0 is off.",
			ULib.ACCESS_ADMIN)
			...
			if getAddonInfo then
				...
				for i = 1, #addons do
					...
					if addon.mounted then
						table.insert(ulx.motdSettings.addons,
							{ title = addon.title, workshop_id = addon.file:gsub("%D", "") })
					end
				end
				...
				table.sort(ulx.motdSettings.addons,
					function(a, b) return string.lower(a.title) < string.lower(b.title) end)
			end
