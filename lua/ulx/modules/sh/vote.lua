...
function ulx.doVote(title, options, callback, timeout, filter, noecho, ...)
	...
	ulx.voteInProgress = { callback = callback, options = options, title = title, results = {}, voters = voters, votes = 0, noecho =
	noecho, args = { ... } }
...
local function voteMapDone(t, argv, ply)
	...
	elseif ply:IsValid() then
		str = "Vote results: Option '" ..
		t.options[winner] .. "' won, changemap pending approval. (" .. winnernum .. "/" .. t.voters .. ")"

		ulx.doVote("Accept result and changemap to " .. changeTo .. "?", { "Yes", "No" }, voteMapDone2, 30000, { ply },
			true, changeTo, ply)
...
local function voteKickDone(t, target, time, ply, reason)
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		elseif ply:IsValid() then
			...
			ulx.doVote("Accept result and kick " .. target:Nick() .. "?", { "Yes", "No" }, voteKickDone2, 30000, { ply },
				true, target, time, ply, reason)
...
local function voteBanDone(t, nick, steamid, time, ply, reason)
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		if ply:IsValid() then
			...
			ulx.doVote("Accept result and ban " .. nick .. "?", { "Yes", "No" }, voteBanDone2, 30000, { ply }, true, nick,
				steamid, time, ply, reason)
		else -- Vote from server console, roll with it
...
function ulx.voteban(calling_ply, target_ply, minutes, reason)
	...
	ulx.doVote(msg, { "Yes", "No" }, voteBanDone, _, _, _, target_ply:Nick(), target_ply:SteamID(), minutes, calling_ply,
		reason)
	if reason and reason ~= "" then
		ulx.fancyLogAdmin(calling_ply, "#A started a voteban of #i minute(s) against #T (#s)", minutes, target_ply,
			reason)
	else
