	...
	for g, limits in ipairs(xgui.data.sboxlimits) do
		if #limits > 0 then
			...
			for _, cvar in ipairs(limits) do
				...
				xgui.queueFunctionCall(xlib.makelabel, "sboxlimits",
					{ x = 10 + (i % 2 * 195), y = 5 + math.floor(i / 2) * 40, w = 185, label = "Max " ..
					cvardata[1]:sub(9), parent = panel })
				xgui.queueFunctionCall(xlib.makeslider, "sboxlimits",
					{ x = 10 + (i % 2 * 195), y = 20 + math.floor(i / 2) * 40, w = 185, label = "<--->", min = 0, max =
					cvardata[2], convar = xlib.ifListenHost(cvardata[1]), repconvar = xlib.ifNotListenHost("rep_" ..
					cvardata[1]), parent = panel, fixclip = true })
				i = i + 1
			end
