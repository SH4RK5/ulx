...
xlib.makebutton { x = 455, y = 6, w = 95, label = "Incomplete: Show", parent = xbans, tooltip = "Filters bans that are loaded by ULib, but do not have any metadata associated with them." }.DoClick = function(
	self)
...
function xgui.ShowBanWindow(ply, ID, doFreeze, isUpdate, bandata)
	...
	xlib.makebutton { x = 45, y = 150, w = 75, label = (isUpdate and "Update" or "Ban!"), parent = xgui_banwindow }.DoClick = function()
		...
		if ULib.isValidSteamID(steamID:GetValue()) then
			...
			if ply then
				RunConsoleCommand("ulx", "ban", "$" .. ULib.getUniqueIDForPlayer(ply), banpanel:GetValue(),
					reason:GetValue())
				xgui_banwindow:Remove()
				return
			end
