...
function groups.init()
	ULib.ucl.registerAccess("xgui_managegroups", "superadmin",
		"Allows managing of groups, users, and access strings via the groups tab in XGUI.", "XGUI")
		...
		if ULib.ucl.query(ply, "ulx addgroup") then
			...
			while group do
				if group == args[1] or args[1] == args[2] then
					ULib.clientRPC(ply, "Derma_Message",
						"Cannot set inheritance! You cannot inherit from something you're inheriting to!", "XGUI NOTICE")
					return
				end
	...
	function groups.refreshTeams()
		...
		if #emptyteams > 0 then
			local output =
			"//This file stores teams that do not have any groups assigned to it (Since ULX would discard them). Do not edit this file!\n"
	...
	if ULib.fileExists("data/ulx/empty_teams.txt") then
		...
		for _, teamdata in ipairs(emptyteams) do
			for k, v in pairs(teamdata) do
				teamdata[k] = tonumber(teamdata[k]) or
				teamdata[k]                              --Ensure any number values are read as numbers and not strings
			end
