...
function cmds.plist:Open(arg)
	if cmds.argslist.secondaryPos == true then
		cmds.argslist:Close()
	elseif self.open then
		self:Close()
	end
			...
			if not xlib.animRunning and not cmds.argslist.open then
				...
				if not cmds.clickedFlag then --Prevent this from happening multiple times.
					...
					xlib.addToAnimQueue(function()
						if not cmds.argslist.open then
							cmds.argslist:Open(ULib.cmds.translatedCmds[cmds.selcmd], false)
						end
						cmds.clickedFlag = nil
					end)
				end
			end
		end
...
function cmds.buildArgsList(cmd)
	...
	local argnum = 0
	local zpos = 0
	local expectingplayers = cmd.args[2] and
	((cmd.args[2].type == ULib.cmds.PlayersArg) or (cmd.args[2].type == ULib.cmds.PlayerArg)) or false
