...
groups.aplayer.DoClick = function()
	...
	for k, v in ipairs(player.GetAll()) do
		if v:GetUserGroup() ~= groups.list:GetValue() then
			menu:AddOption(v:Nick() .. "  |  " .. v:GetUserGroup(),
				function() groups.changeUserGroup(v:SteamID(), groups.list:GetValue()) end)
		end
	end
	menu:AddSpacer()
	for ID, v in pairs(xgui.data.users) do
		if v.group ~= groups.list:GetValue() and not groups.isOnline(ID) then
			menu:AddOption((v.name or ID) .. "  |  " .. (v.group or "<none?>"),
				function() groups.changeUserGroup(ID, groups.list:GetValue()) end)
		end
	end
	menu:AddSpacer()
...
function groups.addBySteamID(group)
	...
	xlib.maketextbox { x = 5, y = 30, w = 180, parent = frame, selectall = true, text = "Enter STEAMID..." }.OnEnter = function(
		self)
...
groups.newgroup.DoClick = function()
	if not ULib.ucl.groups[groups.gname:GetValue()] then
		...
		if groups.gcantarget:GetValue() ~= "" and groups.gcantarget:GetValue() ~= "*" then
			ULib.queueFunctionCall(RunConsoleCommand, "ulx", "setgroupcantarget", groups.gname:GetValue(),
				groups.gcantarget:GetValue())
		end
	else
...
groups.gupdate.DoClick = function(self)
	...
	if groups.gname:GetValue() ~= groupname then
		if groupname == "superadmin" or groupname == "admin" then
			Derma_Query(
				"Renaming the " ..
				groupname .. " group is generally a bad idea, and it could break some plugins. Are you sure?",
				"XGUI WARNING",
...
groups.gdelete.DoClick = function()
	...
	if group == "superadmin" or group == "admin" then
		Derma_Query(
			"Removing the " .. group .. " group is generally a bad idea, and it could break some plugins. Are you sure?",
			"XGUI WARNING",
			"Remove", function()
				RunConsoleCommand("ulx", "removegroup", group)
			end,
			"Cancel", function() end)
	else
...
groups.upbtn.DoClick = function(self)
	...
	RunConsoleCommand("xgui", "updateTeamValue", groups.teamlist.Lines[lineID - 1]:GetColumnText(1), "order", lineID,
		"true")
end
...
groups.teammodifiers.OnRowSelected = function(self, LineID, Line)
	...
	applybtn.DoClick = function()
		if Line:GetColumnText(1) == "color" then
			...
			RunConsoleCommand("xgui", "updateTeamValue", groups.teamlist:GetSelected()[1]:GetColumnText(1),
				Line:GetColumnText(1), col.r, col.g, col.b)
		else
			...
			RunConsoleCommand("xgui", "updateTeamValue", groups.teamlist:GetSelected()[1]:GetColumnText(1),
				Line:GetColumnText(1), groups.teamctrl:GetValue())
		end
	end
...
groups.teammodadd.DoClick = function()
	...
	for _, allowedname in pairs(allowedSorted) do
		...
		if add then
			...
			menu:AddOption(allowedname,
				function() RunConsoleCommand("xgui", "updateTeamValue", team, allowedname, def) end)
		end
	end
	menu:Open()
end
	...
	if ULib.ucl.groups[groups.list:GetValue()] then
		...
		for access, line in pairs(groups.access_lines) do
			...
			if foundAccess then
				...
				foundAccess, fromGroup, restrictionString = groups.checkInheritedAccess(
				ULib.ucl.groups[group].inherit_from, access)
				line.Columns[2]:SetDisabled(foundAccess)
			end
			line.Columns[1]:SetColor((foundAccess and (restrictionString ~= "" and restrictionString ~= "*")) and
			SKIN.text_highlight or SKIN.text_dark)
...
function groups.populateRestrictionArgs(cmd, accessStr)
					...
					outPanel.textbox.OnEnter = function(self)
						if self:GetValue() ~= "" then
							if not (string.find(self:GetValue(), "<") or string.find(self:GetValue(), ">") or string.find(self:GetValue(), ",")) then
								...
								Derma_Message("You cannot use a string that contains the following characters: < > ,",
									"XGUI NOTICE")
							end
						end
					end
	...
	elseif groups.access_lines[cmd].Columns[2].disabled then
		...
		groups.applyInheritedButton.DoClick = function(self)
			RunConsoleCommand("ulx", "groupallow", groups.access_lines[cmd]:GetColumnText(4), cmd,
				groups.generateAccessString())
		end
		groups.applyButton:SetText("Apply access + restrictions")
		groups.applyInheritedButton:SetText("Apply restrictions at \"" ..
		groups.access_lines[cmd]:GetColumnText(4) .. "\" level")
		...
		if panel.GetExpanded then --Weed out panels that we're not interested in
			if panel:GetExpanded() then
				if pnl.type == "ply" then
					...
					outtmp ..
					" " ..
					(pnl.cantarget:GetChecked() and "$" or "") ..
					(pnl.txtfield:GetValue() ~= "" and pnl.txtfield:GetValue() or "*")
					outtmp = ""
				elseif pnl.type == "num" then
					if pnl.hasmin:GetChecked() or pnl.hasmax:GetChecked() then
						...
						outtmp ..
						" " ..
						(pnl.hasmin:GetChecked() and pnl.min:GetValue() or "") ..
						(pnl.hasmax:GetChecked() and ":" .. pnl.max:GetValue() or "")
						outtmp = ""
					else
						outtmp = outtmp .. " *"
					end
		...
		if access == "ulx groupallow" or access == "ulx groupdeny" or access == "ulx userallow" or access == "ulx userdeny" or access == "xgui_managegroups" then
			...
			if foundAccess and fromGroup == group then
				...
				if ULib.ucl.groups[group].inherit_from then
					local foundAccess, fromGroup = groups.checkInheritedAccess(ULib.ucl.groups[group].inherit_from,
						access)
					if foundAccess then
				...
				Derma_Query(
					"WARNING! Removing access to " ..
					access ..
					" will revoke YOUR access to features you're currently using.\nYou will most likely not be able to regain access without console intervention.\nAre you sure you wish to proceed?",
					"XGUI WARNING",
...
function groups.showAccessOptions(line)
	...
	if line.Columns[2]:GetChecked() then
		if line.Columns[2].disabled then
			menu:AddOption("Grant access at \"" .. groups.list:GetValue() .. "\" level",
				function() groups.accessChanged(access, true) end)

			menu:AddOption("Revoke access from \"" .. line:GetColumnText(4) .. "\"",
				function() groups.accessChanged(access, false, line:GetColumnText(4)) end)
		else
